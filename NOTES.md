...
- **Filtering** combines both criteria (AND): search matches `strLeague` _or_ `strLeagueAlternate` (case-insensitive), and the dropdown narrows by exact sport.
...
- **Husky + lint-staged git hooks:** `pre-commit` runs `eslint --fix` + Prettier on staged files only (fast, no full-project lint), and `pre-push` runs the Vitest suite — broken code never leaves the machine. Hooks are installed automatically on `npm install` via the `prepare` script.
